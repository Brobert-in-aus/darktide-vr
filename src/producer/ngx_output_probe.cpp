...
std::uint32_t evaluate_hook(void* commands, const void* feature,
  ...
  if (call <= kCallLimit && samples.load(std::memory_order_relaxed) < kSampleLimit &&
  ...
  if (captured || call <= 4 || call == kCallLimit) {
    ...
    const auto length = std::snprintf(line.data(), line.size(),
        ...
        "get_results=%x,%x,%x,%x,%x output_complete=0 publication=0\n",
...
bool install_ngx_output_probe(HMODULE capture_module) {
  ...
  if (GetFileAttributesW((directory + L"..\\darktidevr_ngx_output_probe.flag").c_str()) ==
  ...
  constexpr char header[] = "ngx_output_probe=armed schema=1 runtime=32.0.16.1088 "
      "resource_get_slot=9 call_limit=32768 sample_limit=256 publication=0\n";
  write_line(header, sizeof(header) - 1);
