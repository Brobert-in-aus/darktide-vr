...
6. **Verify each action family and lifecycle.** Cover hitscan, grouped shots,
   ...
   outside the fixed combat cache. An undeployed camera-only route now follows
   the jump binding (A by default) with independent native rearming and matching
   hints. It preserves local mode admission and stock target decisions. First-person
   ...
   death, safe-zone, hogtied and rescue transitions remain under stock ownership;
   see [spectator ownership](MISSION-INTERACTION-AUDIT.md#spectator-input-and-rescue-ownership-7-september).
