...
|---|---|---|
| One reproducible release candidate | Accepted Lua, native and viewer come from different focused branches. The packager now supports hash-matched component build records, but an accepted candidate with verified clean-build provenance has not been assembled. | Select the component revisions deliberately; build/package them with source revisions, build options, file hashes and dependency versions recorded. Test this exact archive, not a nearby developer installation. |
