...
def row(name):
    ...
    viewer_path = ROOT + name + '/viewer.log'
    try:
        viewer = open(viewer_path, 'rb').read().decode('utf-8', 'replace')
    except FileNotFoundError:
        viewer = ''
    frames = [int(m) for m in re.findall(r'DARKTIDEVR_FRAME_PROFILE report=\d+ frames=(\d+)', console)]
    loop = statistics.mean(frames[2:]) / 5 if len(frames) > 4 else (statistics.mean(frames) / 5 if frames else 0)
...
print('%-28s %-4s %9s %8s %10s %7s %8s %6s' % ('run', 'st', 'gpu/pair', 'loop Hz', 'period ms', 'pairs', 'submit/s', 'errors'))
for name in sys.argv[1:]:
    n, st, gpu, loop, period, pairs, submit, errors = row(name)
    print('%-28s %-4s %9.1f %8.1f %10.1f %7d %8.1f %6d' % (n, st, gpu, loop, period, pairs, submit, errors))
